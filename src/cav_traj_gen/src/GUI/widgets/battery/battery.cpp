/*
 * battery.cpp
 * Function for battery SOC display is defined here.
...
Battery::Battery(QWidget *parent) : QWidget(parent)
{
    ...
    valueLabel = new QLabel(this);  // 创建 QLabel
...
void Battery::drawBorder(QPainter *painter)
{
    ...
    //绘制电池边框
    QPointF topLeft(borderWidth+space, borderWidth+space);
    QPointF bottomRight(batteryWidth, height() - borderWidth-space);
...
void Battery::updateValueLabel()
{
    valueLabel->setGeometry(batteryRect.topLeft().x(), batteryRect.topLeft().y(), batteryRect.width(), batteryRect.height());
    valueLabel->setAlignment(Qt::AlignCenter);
    valueLabel->setStyleSheet("QLabel { background: transparent; color: white; font-weight: bold; }");
    valueLabel->setText(QString::number(static_cast<int>(value)) + "%");
}
...
void Battery::resizeEvent(QResizeEvent *event)
{
    ...
    int newFontSize = height() / 5;  // 根据窗口高度动态计算字体大小
    QFont font = valueLabel->font();
    font.setPointSize(newFontSize);  // 设置新的字体大小
    valueLabel->setFont(font);  // 应用新的字体
